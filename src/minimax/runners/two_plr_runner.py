...
class DualPLRRunner(DRRunner):
	...
	def reset(self, rng):
        ...
        self.repeat_plr_mgr = PopPLRManager(
			n_agents=self.n_students,
			example_level=example_state,
			ued_score=self.ued_score,
			replay_prob=self.replay_prob,
		...
		repeat_plr_buffer = self.repeat_plr_mgr.reset(self.n_students)

		train_state = runner_state[1]
		train_state = train_state.replace(plr_buffer=plr_buffer, repeat_plr_buffer=repeat_plr_buffer)
		if self.n_devices == 1:
	...
	def _eval_and_update_plr(
        ...
        next_repeat_plr_buffer = self.repeat_plr_mgr.update(
			train_state.repeat_plr_buffer, 
			levels=levels, 
			level_idxs=level_idxs, 
			ued_scores=ued_scores,
			dupe_mask=dupe_mask, 
		...
		next_repeat_plr_buffer = jax.vmap(
			lambda update, new, prev: jax.tree_map(
				lambda x, y: jax.lax.select(update, x, y), new, prev)
		)(update_plr, next_repeat_plr_buffer, train_state.repeat_plr_buffer)

		train_state = train_state.replace(plr_buffer=next_plr_buffer, repeat_plr_buffer=nexr_repeat_plr_buffer)
