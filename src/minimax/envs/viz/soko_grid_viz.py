...
from minimax.envs.sokoban.common import OBJECT_TO_INDEX, COLOR_TO_INDEX, COLORS
...
class SokobanGridVisualizer(GridVisualizer):
	@classmethod
	def _render_grid(
		cls,
		grid,
		...
		for y in range(grid.shape[0]):
			for x in range(grid.shape[1]):		
				obj = np.argmax(grid[y,x,:])
				# if obj[0] in [SOK_OBJECT_TO_INDEX['empty']] \
				# 	and obj[2] == 0:
				# 	obj = None

				tile_img = SokobanGridVisualizer._render_tile(
					obj,
	...
	def _render_state(self, params, state, highlight=False, tile_size=TILE_PIXELS, maze_map=None):
		"""
		Render the state
		"""
		...
		img = SokobanGridVisualizer._render_grid(
			grid,
			tile_size,
	...
	def render(self, params, state, highlight=False, tile_size=TILE_PIXELS, maze_map=None):
	...
	def _render_tile(
		...
		if obj is not None and \
			obj == OBJECT_TO_INDEX['player'] and \
			agent_dir_idx is not None:
			obj = np.array(obj)
			

		no_object = obj is None or (
			obj in [OBJECT_TO_INDEX['empty']]
		)
		...
		if not no_object:
			SokobanGridVisualizer._render_obj(obj, img)
